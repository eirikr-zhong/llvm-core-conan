...
class LLVMCoreConan(ConanFile):
    ...
    def _configure_cmake(self):
        ...
        cmake.definitions['CMAKE_POSITION_INDEPENDENT_CODE'] = self.options.get_safe('fPIC', default=True)
        ...
        cmake.definitions['LLVM_ENABLE_PIC'] = \
            self.options.get_safe('fPIC', default=True)
        ...
        cmake.definitions['LLVM_INCLUDE_TOOLS'] = False
        cmake.definitions['LLVM_BUILD_TOOLS'] = False
        ...
        cmake.definitions['LLVM_ENABLE_ASSERTIONS'] = self.options.shared
        ...
        cmake.definitions['LLVM_ENABLE_ZSTD'] = False
        return cmake
    ...
    def source(self):
        git = Git(self)
        git.clone(url="https://github.com/llvm/llvm-project.git", args=['--depth', '1', '--branch', "llvmorg-" + self.version])

    def build(self):
        cmake = self._configure_cmake()
        source = self.source_path.joinpath('llvm-project').joinpath('llvm')
        cmake.configure(source_dir=str(source))
        cmake.build()
